...
This sample shows the smallest useful Zafiro shell setup with a two-level section tree.
...
- `App.axaml.cs` registers the shell with `AddZafiroShell()` and discovers sections with `AddAllSectionsFromAttributes()`.
...
Add a ViewModel and a matching View under `Sections/`, then decorate the ViewModel:
