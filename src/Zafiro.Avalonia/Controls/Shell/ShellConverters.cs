...
public static class ShellConverters
{
    ...
    public static readonly FuncMultiValueConverter<object, bool> ShouldShowSelectedChildLevel = new(values =>
    {
        var list = values.ToList();

        return list is [ISection currentSection, ISection selectedRoot, IReadOnlyList<SectionLevel> childLevels]
            && ReferenceEquals(currentSection, selectedRoot)
            && childLevels.FirstOrDefault()?.Sections is { Count: > 1 };
    });

    public static readonly FuncMultiValueConverter<object, bool> ShouldShowSelectedRootWithoutChildLevel = new(values =>
    {
        var list = values.ToList();

        return list is [ISection currentSection, ISection selectedRoot, IReadOnlyList<SectionLevel> childLevels]
            && ReferenceEquals(currentSection, selectedRoot)
            && childLevels.FirstOrDefault()?.Sections is not { Count: > 1 };
    });
}
